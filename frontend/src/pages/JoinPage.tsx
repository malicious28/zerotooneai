  ...
  if (error && !group) return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-blue-100">
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
        ...
        <button onClick={() => navigate('/login')} className="mt-6 text-indigo-600 hover:underline text-sm">Go to login</button>
      </div>
    </div>
  )
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
        <div className="mb-6 text-center">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Join <span className="font-semibold text-indigo-600">{group?.name}</span> on AudienceBuilder
          </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input value={name} onChange={e => setName(e.target.value)} required className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="Your name" />
          </div>
          <div>
          ...
          <button type="submit" disabled={submitting} className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white font-medium py-2 rounded-lg transition-colors">
        ...
        <p className="text-center text-xs text-gray-400 mt-4">
          ...
          <button onClick={() => navigate('/login')} className="text-indigo-500 hover:underline">Sign in</button>
        </p>
      </div>
    </div>
  )
